...
class CKException: public std::exception {
    public:
        const char* message;
        CKException(const char* msg) : message(msg) {}
        const char* what() {
            return this->message;
        }
};
...
struct CKRect {
    ...
    CKRect(int x, int y, int w, int h): x(x), y(y), width(w), height(h) { };
    CKRect(int w, int h): x(0), y(0), width(w), height(h) { };
    CKRect(): x(0), y(0), width(0), height(0) { };
...
struct CKControlEvent {
    ...
    CKControlEvent(CKControlEventType type): type(type) { } 
    CKControlEvent(CKControlEventType type, CKPoint point): type(type), point(point) { }
